...
--Q1
Select name, cardNum
from [User] join [UserCard] on [User].[userId] = UserCard.userID

--Q2
SELECT cardNum, balance, name
FROM [Card] join [CardType] on Card.cardTypeID = CardType.cardTypeID

--Q3
SELECT [Card].cardNum, transDate, amount
FROM [Card] left outer join [Transaction] on [Card].cardNum = [Transaction].cardNum

--Q4
SELECT [User].userId,[User].name, [Card].cardNum, [Transaction].amount
FROM (([Card] left outer join [Transaction] on [Card].cardNum = [Transaction].cardNum) right outer join [UserCard] on [UserCard].cardNum = [Card].cardNum) right outer join [User] on [User].userID = [UserCard].userID

--Q5
SELECT [User].userId,[User].name
FROM [User] left outer join [UserCard] on [User].userId = [UserCard].userID
WHERE [UserCard].[cardNum] is NULL

--Q6
SELECT name,[Card].cardNum
FROM ([User] join [UserCard] on [User].[userId] = [UserCard].[userID]) join [Card] on [Card].[cardNum] = [UserCard].[cardNum]
WHERE  20000 < balance AND balance < 40000 

--Q7
SELECT [User].userId,name,[Card].cardNum, phoneNum, city, expireDate
FROM ([User] join [UserCard] on [User].[userId] = [UserCard].[userID]) join [Card] on [Card].[cardNum] = [UserCard].[cardNum]
WHERE DATEDIFF(MONTH,GETDATE(),expireDate) = 3